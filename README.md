...
Go source files with directives such as:

```go
// #cgo CFLAGS: -I/opt/local/lib/ImageMagick7/include/ImageMagick-7
// #cgo LDFLAGS: -L/opt/local/lib/ImageMagick7/lib -lMagickWand-7.Q16HDRI -lMagickCore-7.Q16HDRI
```

However, the ImageMagick Go wrapper I'm using is split across versions — you basically include version one for 'legacy' ImageMagick versions, version two for ImageMagick 6.9.X, and version three for ImageMagick 7.X. It's a bit awkward, but it works — that means the Go code is never touched, you just switch to the appropriate version, depending on what you have on your system.

In the future, what I need to do is to automate all of that: Go most definitely allows, between those directives and using tags, selecting the 'correct' libraries during the build, without human intervention. All I need to do is to set things up properly (and no, no `makefile` is needed for _that_; it's much more akin how the C pre-processor works). This hasn't been done yet, since it _might_ be far easier to simply fork an instance of ImageMagick in the background.

And who knows, perhaps one day someone will finish the very difficult task of porting the JPEG2000 decoding algorithm to native Go. It's really not for the faint of heart (I _had_ a look at it)!
...
[![gOSWI build with Debian packages](https://github.com/GwynethLlewelyn/goswi/actions/workflows/go.yml/badge.svg)](https://github.com/GwynethLlewelyn/goswi/actions/workflows/go.yml) [![Codacy Security Scan](https://github.com/GwynethLlewelyn/goswi/actions/workflows/codacy.yml/badge.svg)](https://github.com/GwynethLlewelyn/goswi/actions/workflows/codacy.yml) [![Go Report Card](https://goreportcard.com/badge/github.com/GwynethLlewelyn/goswi)](https://goreportcard.com/report/github.com/GwynethLlewelyn/goswi) [![CodeQL](https://github.com/GwynethLlewelyn/goswi/actions/workflows/codeql-analysis.yml/badge.svg)](https://github.com/GwynethLlewelyn/goswi/actions/workflows/codeql-analysis.yml)
[![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=GwynethLlewelyn_goswi&metric=alert_status)](https://sonarcloud.io/summary/new_code?id=GwynethLlewelyn_goswi) [![FOSSA Status](https://app.fossa.com/api/projects/git%2Bgithub.com%2FGwynethLlewelyn%2Fgoswi.svg?type=shield)](https://app.fossa.com/projects/git%2Bgithub.com%2FGwynethLlewelyn%2Fgoswi?ref=badge_shield) [![DeepSource](https://app.deepsource.com/bb/GwynethLlewelyn/goswi.svg/?label=active+issues&show_trend=true&token=oHDSHTw5H7exBcoc7FsKf1Vz)](https://app.deepsource.com/bb/GwynethLlewelyn/goswi/)
